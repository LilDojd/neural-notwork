...
def embed_in_grid(features, pdb_id, output_dir,
    ...
    for residue_index in np.unique(features[['res_index']].view(int)):
        ...
        if coordinate_system == grid.CoordinateSystem.spherical:
            ...
            # Create grid
            grid_matrix = grid.create_spherical_grid(max_radius=max_radius, n_features=n_features,
            ...
            # Create grid
            grid_matrix = grid.create_cubed_sphere_grid(max_radius=max_radius, n_features=n_features,
                                                        bins_per_angstrom=bins_per_angstrom)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--coordinate-system", choices=[e.name for e in grid.CoordinateSystem],
                        default=grid.CoordinateSystem.spherical.name,
                        help="Which coordinate system to use (default: %(default)s)")
    parser.add_argument("--z-direction", choices=[e.name for e in grid.ZDirection],
                        default=grid.ZDirection.outward.name,
    ...
    if args.mode == "fetch-and-extract":
        ...
        joblib.Parallel(n_jobs=args.n_proc, batch_size=1)(
            joblib.delayed(extract_atomistic_features)(pdb_filename,
                                                       ...
                                                       z_direction=z_direction,
                                                       include_center=args.include_center,
                                                       dssp_executable=args.dssp_executable) for pdb_filename in
            pdb_filenames)
    else:
