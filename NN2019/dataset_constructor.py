...
for name, group in data.groupby(list(range(1, extract_at_level + 1))):
    ...
    # Select elements with at least min_arch_size structures with resolution at least min_resolution (note higher
    # resolution is smaller number) and with specified maximum distance to CM.
    if len(group[np.logical_and(group[11] < args.min_resolution, group['dist'] < args.max_distance)]) > args.min_size:
        ...
        sub_category_groups = group[
            np.logical_and(group[11] < args.min_resolution, group['dist'] < args.max_distance)].groupby(
            sub_category_col_range)
...
np.savez_compressed(
    "cath_%d%s_%s" % (len(set(labels)), args.extract_at_level, args.atom_selector_regexp.replace("|", "")),
